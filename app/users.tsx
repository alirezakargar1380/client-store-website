...
export default function Users({ products, category }: any) {
  const [data, setData] = useState(products);
  const [landingSection, setLandingSection] = useState(category);

  const handleClick = (event: any, id: any) => {
    event.preventDefault();
    if (id) {
      fetch(`http://localhost:3002/api/products?data_per_page=10&page=1&sort=1&category_parent=3`).then(async (res) => {
                ...
                <div className="container new-arrivals" key={index}>
                      ...
                      <ul className="nav nav-pills nav-border-anim justify-content-center" role="tablist">
                        ...
                        {item.landingSectionCategory.map((item2: any, index2: number) => {
                          if (index2 === 0) {
                            return (
                              <div
                                onClick={(e) => { handleClick(e, item2.category?.id) }}
                              >
                                <li className="nav-item">
                                  <a
                                    className="nav-link active"
                                    id={"new-all-link" + item.id}
                                    data-toggle="tab"
                                    href="#new-all-tab"
                                    role="tab"
                                    aria-controls="new-all-tab"
                                    aria-selected="true"
                                  >{(item2.category) ? item2.category.name : "همه"}</a>
                                </li>
                              </div>
                  ...
                  <div className="tab-content tab-content-carousel just-action-icons-sm">
                    ...
                    <div className="tab-pane p-0 fade show active"
                      ...
                      aria-labelledby={"new-all-link" + item.id}
                    >
